...
function tagValue(tags, name) {
    for (const tag of tags) {
        if (encoding_1.fromB64Url(tag.name).toString().toLowerCase() === name.toLowerCase()) {
            return encoding_1.fromB64Url(tag.value).toString();
        }
    }
    return '';
}
...
function tagToUTF8(tags) {
    ...
    for (const tag of tags) {
        conversion.push({
            name: encoding_1.fromB64Url(tag.name).toString(),
            value: encoding_1.fromB64Url(tag.value).toString(),
        });
    }
    return conversion;
}
