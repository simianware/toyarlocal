...
function formatTransaction(transaction) {
    try {
        ...
        return lodash_1.pick(Object.assign(Object.assign(Object.assign({}, transaction), indexFields), { content_type: utils_1.Utils.tagValue(transaction.tags, 'content-type'), format: transaction.format || 0, data_size: transaction.data_size || (transaction.data ? encoding_1.fromB64Url(transaction.data).byteLength : undefined), tags: JSON.stringify(transaction.tags), owner_address: encoding_1.sha256B64Url(encoding_1.fromB64Url(transaction.owner)) }), exports.transactionFields.concat(order_1.indices));
    }
    catch (error) {
        console.error({ error });
    }
...
function formatAnsTransaction(ansTransaction) {
    ...
    return lodash_1.pick(Object.assign(Object.assign({}, indexFields), { id: ansTransaction.id, owner: ansTransaction.owner, content_type: 'ANS-102', target: ansTransaction.target, tags: JSON.stringify(ansTransaction.tags) }), exports.transactionFields.concat(order_1.indices));
}
...
class TransactionDB {
    ...
    mineTxs(block) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.connection('transactions').where({ block: '' }).update({ block });
        });
    }
}
